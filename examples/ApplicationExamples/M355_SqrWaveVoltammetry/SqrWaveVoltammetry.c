...
uint8_t totalDataReceivedSQW1=0;
...
static AD5940Err AppSWVSeqInitGen(void)
{
  ...
  if(AppSWVCfg.LPTIARtiaSel == LPTIARTIA_OPEN) /* User want to use external RTIA */
    ...
    lploop_cfg.LpAmpCfg.LpTiaSW = LPTIASW(13)|LPTIASW(2)|LPTIASW(4);// https://ez.analog.com/data_converters/precision_adcs/f/q-a/556667/m355_sqrwavevoltammetry
...
//**********************Measure DPV response******************************/
//*******Config ADC to READ SE0 on click sinq0 ( LPTIA0)******************/
...
//********************Measure output ramp voltage*************************/
//*******Config ADC to READ ADCMUXP_VRE0 on click sinq0 ******************/
	AD5940_StructInit(&dsp_cfg, sizeof(dsp_cfg));
	dsp_cfg.ADCBaseCfg.ADCMuxN = ADCMUXN_VZERO0;
	dsp_cfg.ADCBaseCfg.ADCMuxP = ADCMUXP_VRE0;
	dsp_cfg.ADCBaseCfg.ADCPga = AppSWVCfg.AdcPgaGain;
...
static int32_t AppSWVDataProcess(int32_t * const pData, uint32_t *pDataCount)
{
 uint32_t i, datacount;
  ...
  {
		pData[i] &= 0xffff;
		temp1 = AD5940_ADCCode2Volt(pData[i], AppSWVCfg.AdcPgaGain, AppSWVCfg.ADCRefVolt);
		pOut[i] = temp1;
		i++;
...
void TurnOffCE_RE_WE()
{
		...
		lp_cfg.LpDacCfg.LpdacSel = LPDAC0;                  /* Select LPDAC0. Note LPDAC1 is available on ADuCM355 */               /* 6Bit DAC data */
